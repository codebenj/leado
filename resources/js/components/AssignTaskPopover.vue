...
export default {
    ...
    mounted() {
        ...
        Bus.$on( 'update-state-task', event => {
            if ( event.task_id && event.task_id == this.id ) this.fetchAssignedUsers( event.task_id )
        } )
    },
	...
	methods: {
        async initData() {
            ...
            let ids = []
            await this.$store.dispatch( 'scrum/fetchAssignedUsers', this.id ).then( res => {
                let data = res
        ...
        async fetchAssignedUsers( id ) {
            let ids = []
            await this.$store.dispatch( 'scrum/fetchAssignedUsers', id ).then( res => {
                let data = res
        ...
        async assignUser( id, userData ) {
            ...
            ids.push( id )
            await this.$store.dispatch('scrum/assignUserToTask', { task_id: this.id, ids: ids }).then( res => {
                if ( res ) {
                    this.usersAssigned.push( userData )
                    Bus.$emit( 'update-state-task', { ids: ids, task_id: this.id } )
                }
            } )
        },
        ...
        removeAssignedUser( user_id ) {
            ...
            this.$store.dispatch('scrum/removeAssignedUser', { task_id: this.id, ids: ids }).then( res => {
                if ( res ) {
                    ...
                    Bus.$emit( 'update-state-lead', { ids: ids, task_id: this.id } )
                }
            } )
        },
