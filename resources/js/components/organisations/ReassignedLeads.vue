<template>
  <div>
    <data-tables-server
      :data="reassigned_leads"
      :total="reassigned_leads.length"
      :loading="loading"
    ...
    <!-- <template v-if="historyDrawerId && user.user_role.name == 'administrator'">
      <el-drawer
...
export default {
  ...
  computed: mapGetters({
    user: "auth/user",
    reassigned_leads: 'organisations/reassigned_leads',
